...
Self-contained authentication components, hooks, and types for the appointment-scheduler application. Built with React, Next.js, Better Auth, React Hook Form, and Zod.
...
```bash
npm install react react-dom next react-hook-form @hookform/resolvers zod zustand better-auth sonner lucide-react
```
...
| Parameter  | Type                                | Required | Description     |
| ---------- | ----------------------------------- | -------- | --------------- |
| `provider` | `'google' \| 'github' \| 'discord'` | Yes      | Social provider |
